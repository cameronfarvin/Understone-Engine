               ...
               size_t* restrict           return_file_size)
{
    ...
    if(!spir_v_file) { uFatal("[ vulkan ] Unable to open SPIR-V file: %s\n", file_name); }
    ...
    *return_file_data      = ( char* )calloc(1, allocation_size);
    if(!(*return_file_data)) { uFatal("[ vulkan ] Unable to allocate host memory for SPIR-V file: %s.\n", file_name); }
...
uLoadSpirvModules(uVulkanShader** const restrict return_shaders, u32* const restrict num_return_shaders)
{
    ...
    uAssertMsg_v(!(*num_return_shaders),
                 "[ shaders ] Number of return shaders "
                 "must be zero; will be overwritten.\n");
